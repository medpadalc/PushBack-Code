...
void tunePid() {
    ...
    subsystems::wing::retract();
    chassis.moveToPose(-19, -34.75, 270, 3500, {.forwards = false, .maxSpeed = 95, .minSpeed = 60, .earlyExitRange = 4});
    chassis.waitUntil(6);
    chassis.cancelMotion();
...
void skillsStart(){
    ...
    */
    ...
    chassis.waitUntilDone();
    subsystems::intake::run(subsystems::intake::GoalType::MEDIUM_GOAL);
    pros::delay(1800); //2000 to 1800 decrease if need more time to 1700
    subsystems::intake::run(subsystems::intake::GoalType::HOLD_BALLS);
    ...
    chassis.waitUntil(10);
    chassis.cancelMotion();
    chassis.moveToPoint(-67.5, 44.5, 450, {.maxSpeed = 50, .minSpeed = 40, .earlyExitRange = 1.5});
    
    // facing goal
    chassis.waitUntilDone();
    chassis.tank(16, 16);
    // in matchloader
    pros::delay(550); //800 to 550
    chassis.tank(0,0);
...
void skillsTwo(){  
    ...
    chassis.moveToPoint(-33, -51, 1500, {.forwards = false, .maxSpeed = 90, .minSpeed = 50, .earlyExitRange = 1.5});
    chassis.moveToPoint(-20, -49.5, 600, {.forwards = false, .maxSpeed = 90, .minSpeed = 50,  .earlyExitRange = 0.5});
...
void skillsThree(){
    ...
    //facing goal
    chassis.moveToPoint(68, -49, 1600, {.maxSpeed = 55, .minSpeed = 50, .earlyExitRange = 1.5});
    subsystems::matchload::extend(); // can take out?

    chassis.waitUntil(20);
    chassis.cancelMotion();
    chassis.moveToPoint(68, -47, 1000, {.maxSpeed = 40, .minSpeed = 40, .earlyExitRange = 1.5}); //67.85 to 68 //untested line

    chassis.waitUntilDone();

    // into matchloader 2
    chassis.tank(15,15);
    pros::delay(1100); //1150 to 1100
    chassis.tank(0,0);

    //out of matchloader 2 and into long goal
    chassis.moveToPoint(25, -48, 1000, {.forwards = false, .maxSpeed = 60, .earlyExitRange = 0.5});
    chassis.waitUntil(22);
    //outtake balls
    subsystems::intake::run(subsystems::intake::GoalType::LONG_GOAL);
    chassis.waitUntilDone();
    chassis.tank(-127,-127);
    pros::delay(2050); //1800 to 2050
    chassis.tank(0,0);

    //reset before hood push (added)
    // subsystems::wing::extend();
    // subsystems::matchload::retract();
    // chassis.setPose(28, 44, 90);

    // hood push
    chassis.moveToPoint(30, -48, 1100, {.maxSpeed = 75, .minSpeed = 60, .earlyExitRange = 0.5}); //first num from 33 to 30
    chassis.moveToPoint(20, -48, 1100, {.forwards = false, .maxSpeed = 13}); //first num from 25 to 20 and 850 to 1100
    chassis.waitUntilDone();
...
void skillsFour() {
    ...
    */
    ...
    chassis.moveToPoint(58, 30, 2000, {.maxSpeed = 65}); // 60.5 to 58
    chassis.waitUntil(15);
    subsystems::intake::run(subsystems::intake::GoalType::HOLD_BALLS);
    chassis.waitUntilDone();
    ...
    // clear park zone
    chassis.moveToPoint(68, 25, 1000, {.maxSpeed = 70, .minSpeed = 70, .earlyExitRange = 1.5});
    chassis.moveToPoint(68, -12.75, 4000, {.maxSpeed = 75}); //69 to 68
    chassis.waitUntilDone();
    pros::delay(400);
    ...
    pros::delay(100);
    // getting extra ball
    chassis.moveToPose(35.75, -22, 270, 4000);
    chassis.waitUntilDone();
    ...
    subsystems::matchload::extend();
    chassis.moveToPoint(11, -12, 2000, {.forwards = false, .maxSpeed = 40});
    chassis.waitUntilDone();
    ...
    chassis.turnToPoint(68, -50.5, 1600, {.maxSpeed = 60, .minSpeed = 60, .earlyExitRange = 1.5});


    chassis.moveToPoint(68, -50.5, 1000, {.maxSpeed = 60, .minSpeed = 60, .earlyExitRange = 1.5});

    chassis.waitUntil(10);
    chassis.cancelMotion();
    chassis.moveToPoint(68.25, -50.5, 525, {.maxSpeed = 50, .minSpeed = 40, .earlyExitRange = 1.5});

    chassis.waitUntilDone();
    chassis.tank(20,20);
    // into 3rd matchloader
    pros::delay(1150); //1050 to 1150
    chassis.tank(0,0);
    chassis.moveToPoint(58, -48.5, 1500, {.forwards = false, .maxSpeed = 80, .earlyExitRange = 0.5});
    chassis.moveToPoint(35, -59.5, 1500, {.forwards = false, .maxSpeed = 90, .minSpeed = 40, .earlyExitRange = 1});

    chassis.moveToPoint(-30, -60.5, 2000, {.forwards = false, .maxSpeed = 100, .minSpeed = 40, .earlyExitRange = 1.5});

    chassis.moveToPoint(-38, -52, 1500, {.forwards = false, .maxSpeed = 90, .minSpeed = 50, .earlyExitRange = 1.5});
    subsystems::matchload::retract();
...
void skillsFive(){
    ...
    chassis.moveToPoint(57, 34, 2000, {.maxSpeed = 60}); //57.25 to 57
    chassis.waitUntil(15);
    subsystems::intake::run(subsystems::intake::GoalType::HOLD_BALLS);

    chassis.moveToPoint(62.5, 11.5, 4000, {.minSpeed = 127}); // 65.5 to 62.5 and 7.75 to 11.5
    pros::delay(450);
    subsystems::matchload::extend();
    pros::delay(1200);
    subsystems::matchload::retract();
